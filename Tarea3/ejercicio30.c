...
taylor, e imprime el resultado del coseno para esa x
...
// Inicio función GetCos, que va a hacer la suma de los terminos indicados para sacar el valor de x dado
double GetCos(double x, int limit, int k) {
    if (k <= limit) { // Mientras el limite inferior no sea mayor al limite superior se continúa con la recursión
        return ((pow(-1, k) * pow(x, 2 * k)) / GetFactorial(2 * k)) + GetCos(x, limit, k + 1); // Llamado recursivo
...
int main() {
    ...
    // Llamamos a la función GetCos e imprimimos el valor que devuelve
    printf("%lf", GetCos(x, limit, 0));
